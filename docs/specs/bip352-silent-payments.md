# BIP-352 Silent Payments — Primitives Implementation Plan
...
Reference: `/Users/sachinmeier/Downloads/bip-0352.mediawiki` (BIP-352 v1.1.1).
...
Per personal convention, prefix titles `SN:` (S = Silent payments):

0. **S0: port BIP-340/341 negation foundation from fork** — bring
   `Point.negate/1` + `PrivateKey.negate/1` (and `Signature.to_hex/1`, `PrivateKey.to_hex/1`)
   from `SachinMeier/bitcoinex@master` into `point.ex` / `privatekey.ex` / `secp256k1.ex`,
   with their unit tests. Small, self-contained; unblocks label matching in S1.
1. **S1: BIP-352 crypto primitives** — `input_hash`, `shared_secret`, `shared_secret_tweak`,
   `sum_input_privkeys`, `sum_input_pubkeys`, `label_tweak`, `label_point` (using the ported
   `Point.negate/1`), with focused unit tests.
2. **S2: send/scan/spend + labels** — `create_output_pubkey`, `scan_output`,
   `spending_privkey`, `create_labeled_spend_pubkey`, `output_label_candidates`, driven by
   the BIP-352 send/receive test vectors (incl. labels and change label).
3. **S3: silent-payment address codec** — `encode_address` / `decode_address` + vectors +
   `stream_data` properties. Update `CHANGELOG.md` and bump `version` in `mix.exs`.
