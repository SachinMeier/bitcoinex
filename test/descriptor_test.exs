defmodule Bitcoinex.DescriptorTest do
  ...
  # >15 are original examples and edge cases
  ...
  @all_descriptors @pk_descriptors ++
                     @pkh_descriptors ++
                     @wpkh_descriptors ++
                     @sh_descriptors ++
                     @wsh_descriptors ++
  ...
  describe "create descriptor" do
    test "create p2pk" do
      ...
      assert Descriptor.serialize_descriptor(desc) ==
               "pk(0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798)"
      ...
      dkey = Descriptor.DKey.from_key(priv, :mainnet)
      {:ok, desc} = Descriptor.create_p2pk(dkey)
    ...
    test "create p2pkh" do
      ...
      # ex13 from @pkh_descriptors
      {:ok, mfp} = Base.decode16("d34db33f", case: :lower)
      ...
      {:ok, anc_path} = DerivationPath.from_string("44'/0'/0'")
      {:ok, desc_path} = DerivationPath.from_string("1/*")

      dkey =
        Descriptor.DKey.from_key(xpub, %{
          parent_fingerprint: mfp,
          anc_path: anc_path,
          desc_path: desc_path
        })
      ...
      # ex20 from @pkh_descriptors
      {:ok, priv, :mainnet, _compressed} =
        PrivateKey.parse_wif("KwDiBf89QgGbjEhKnhXJuH7LrciVrZi4ZxKRdkhWeLbjoGkhRF5E")

      priv1 = PrivateKey.wif!(priv, :mainnet)
      assert priv1 == "KwDiBf89QgGbjEhKnhXJuH7LrciVrZi4ZxKRdkhWeLbjoGkhRF5E"
      ...
      # ex21 from @pkh_descriptors
      {:ok, priv, :testnet, _compressed} =
        PrivateKey.parse_wif("cMahea7zqjxrtgAbB7LSGbcQUr1uX1okdzTtkBA29TFk41r74ddm")

      dkey = Descriptor.DKey.from_key(priv, :testnet)
      {:ok, desc} = Descriptor.create_p2pkh(dkey)
    ...
    test "create p2sh" do
      ...
      {:ok, wpkh_desc} = Descriptor.create_p2wpkh(pk)
      {:ok, sh_desc} = Descriptor.create_p2sh(wpkh_desc)

      assert Descriptor.serialize_descriptor(sh_desc) ==
               "sh(wpkh(03fff97bd5755eeea420453a14355235d382f6472f8568a18b2f057a1460297556))"
      ...
      {:ok, pkh_desc} = Descriptor.create_p2pkh(pk)
      {:ok, wsh_desc} = Descriptor.create_p2wsh(pkh_desc)
      {:ok, sh_desc} = Descriptor.create_p2sh(wsh_desc)
      ...
      {:ok, multi_desc} = Descriptor.create_multi({2, [pk1, pk2]})
      {:ok, sh_desc} = Descriptor.create_p2sh(multi_desc)
      ...
      {:ok, multi_desc} = Descriptor.create_sortedmulti({2, [pk1, pk2]})
      {:ok, sh_desc} = Descriptor.create_p2sh(multi_desc)
      ...
      {:ok, multi_desc} = Descriptor.create_multi({1, [pk1, pk2, pk3]})
      {:ok, wsh_desc} = Descriptor.create_p2wsh(multi_desc)
      {:ok, sh_desc} = Descriptor.create_p2sh(wsh_desc)
    ...
    test "create p2wpkh" do
      # ex2 describes a P2WPKH output with the specified public key.
      {:ok, pk} =
        Point.parse_public_key(
          "02f9308a019258c31049344f85f89d5229b531c845836f99b08601f113bce036f9"
        )

      {:ok, desc} = Descriptor.create_p2wpkh(pk)

      assert Descriptor.serialize_descriptor(desc) ==
               "wpkh(02f9308a019258c31049344f85f89d5229b531c845836f99b08601f113bce036f9)"

      # ex21 describes a P2WPKH to a mainnet private key
      {:ok, priv, :mainnet, _compressed} =
        PrivateKey.parse_wif("KwDiBf89QgGbjEhKnhXJuH7LrciVrZi4ZxKRdkhWeLbjoGkhRF5E")

      dkey = Descriptor.DKey.from_key(priv, :mainnet)
      {:ok, desc} = Descriptor.create_p2wpkh(dkey)

      assert Descriptor.serialize_descriptor(desc) ==
               "wpkh(KwDiBf89QgGbjEhKnhXJuH7LrciVrZi4ZxKRdkhWeLbjoGkhRF5E)"

      # ex22 describes a P2WPKH to a testnet private key
      {:ok, priv, :testnet, _compressed} =
        PrivateKey.parse_wif("cMahea7zqjxrtgAbB7LSGbcQUr1uX1okdzTtkBA29TFk41r74ddm")
    ...
    test "create p2wsh" do
      ...
      {:ok, multi_desc} = Descriptor.create_multi({2, [pk1, pk2, pk3]})
      {:ok, wsh_desc} = Descriptor.create_p2wsh(multi_desc)

      assert Descriptor.serialize_descriptor(wsh_desc) ==
               "wsh(multi(2,03a0434d9e47f3c86235477c7b1ae6ae5d3442d49b1943c2b752a68e2a47e247c7,03774ae7f858a9411e5ef4246b70c65aac5649980be5c17891bbec17895da008cb,03d01115d548e7561b15c38f004d734633687cf4419620095bc5b0f47070afe85a))"
      ...
      {:ok, deriv1} = ExtendedKey.DerivationPath.from_string("1/0/*")
      ...
      {:ok, deriv2} = ExtendedKey.DerivationPath.from_string("0/0/*")
      dkey1 = Descriptor.DKey.from_key(xpub1, %{desc_path: deriv1})
      dkey2 = Descriptor.DKey.from_key(xpub2, %{desc_path: deriv2})
      {:ok, multi_desc} = Descriptor.create_multi({1, [dkey1, dkey2]})
      {:ok, wsh_desc} = Descriptor.create_p2wsh(multi_desc)

      assert Descriptor.serialize_descriptor(wsh_desc) ==
               "wsh(multi(1,xpub661MyMwAqRbcFW31YEwpkMuc5THy2PSt5bDMsktWQcFF8syAmRUapSCGu8ED9W6oDMSgv6Zz8idoc4a6mr8BDzTJY47LJhkJ8UB7WEGuduB/1/0/*,xpub69H7F5d8KSRgmmdJg2KhpAK8SR3DjMwAdkxj3ZuxV27CprR9LgpeyGmXUbC6wb7ERfvrnKZjXoUmmDznezpbZb7ap6r1D3tgFxHmwMkQTPH/0/0/*))"

      # 15 describes a set of 1-of-2 P2WSH multisig outputs where one multisig key is the 1/0/i child of the first specified xpub and the other multisig key is the 0/0/i child of the second specified xpub, and i is any number in a configurable range (0-1000 by default).
      {:ok, xpub1} =
        ExtendedKey.parse_extended_key(
          "xpub661MyMwAqRbcFW31YEwpkMuc5THy2PSt5bDMsktWQcFF8syAmRUapSCGu8ED9W6oDMSgv6Zz8idoc4a6mr8BDzTJY47LJhkJ8UB7WEGuduB"
        )
      ...
      dkey1 = Descriptor.DKey.from_key(xpub1, %{desc_path: deriv1})
      dkey2 = Descriptor.DKey.from_key(xpub2, %{desc_path: deriv2})
      {:ok, multi_desc} = Descriptor.create_sortedmulti({1, [dkey1, dkey2]})
      {:ok, wsh_desc} = Descriptor.create_p2wsh(multi_desc)
    ...
    test "create combo" do
      ...
      {:ok, desc} = Descriptor.create_combo(pk)

      assert Descriptor.serialize_descriptor(desc) ==
               "combo(0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798)"
    end

    test "create multi" do
      # 6 describes a bare 1-of-2 multisig output with keys in the specified order.
      {:ok, pk1} =
        Point.parse_public_key(
      ...
      {:ok, desc} = Descriptor.create_multi({1, [pk1, pk2]})
      ...
      {:ok, desc} = Descriptor.create_multi({2, [pk1, pk2, pk3]})
      ...
      dkey1 = Descriptor.DKey.from_key(xpub1, %{desc_path: deriv1})
      dkey2 = Descriptor.DKey.from_key(xpub2, %{desc_path: deriv2})
      {:ok, desc} = Descriptor.create_multi({1, [dkey1, dkey2]})

      assert Descriptor.serialize_descriptor(desc) ==
               "multi(1,xpub661MyMwAqRbcFW31YEwpkMuc5THy2PSt5bDMsktWQcFF8syAmRUapSCGu8ED9W6oDMSgv6Zz8idoc4a6mr8BDzTJY47LJhkJ8UB7WEGuduB/1/0/*,xpub69H7F5d8KSRgmmdJg2KhpAK8SR3DjMwAdkxj3ZuxV27CprR9LgpeyGmXUbC6wb7ERfvrnKZjXoUmmDznezpbZb7ap6r1D3tgFxHmwMkQTPH/0/0/*)"
    end

    test "create sortedmulti" do
      # taken from 15, describes a set of 1-of-2 P2WSH multisig outputs where one multisig key is the 1/0/i child of the first specified xpub and the other multisig key is the 0/0/i child of the second specified xpub, and i is any number in a configurable range (0-1000 by default). The order of public keys in the resulting witnessScripts is determined by the lexicographic order of the public keys at that index.
      {:ok, xpub1} =
        ExtendedKey.parse_extended_key(
          "xpub661MyMwAqRbcFW31YEwpkMuc5THy2PSt5bDMsktWQcFF8syAmRUapSCGu8ED9W6oDMSgv6Zz8idoc4a6mr8BDzTJY47LJhkJ8UB7WEGuduB"
      ...
      dkey1 = Descriptor.DKey.from_key(xpub1, %{desc_path: deriv1})
      dkey2 = Descriptor.DKey.from_key(xpub2, %{desc_path: deriv2})
      {:ok, desc} = Descriptor.create_sortedmulti({1, [dkey1, dkey2]})
